...
- **Centre Ring** — an alternative view built around a single spotlight. Otto stands in it with a line showing what he's doing right now; sub-agents walk into view as crew when they start work and take a bow when they finish; each turn becomes a bar in a strip along the bottom, sized by how long it took. The spotlight turns gold whenever the session needs a decision from you, with permission, question, and plan prompts stepping into it to ask. Switch between Default and Ring from the session header — your choice is remembered, and phones and small screens always show Default
